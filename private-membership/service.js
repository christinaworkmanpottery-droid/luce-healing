...
function createMembership({pool,getMailer,env=process.env,clock=()=>new Date(),stripeClient}){
 ...
 function register(app,checkAdmin){
  ...
  app.get('/api/membership/places',needPreview,wrap(async(req,res)=>{await needMember(req);await limit('places:'+req.socket.remoteAddress,300);res.json(chart.searchPlaces(String(req.query.q||'').slice(0,100)));}));
  ...
  admin('post','/horoscopes/:month/unpublish',async(req,res)=>{const result=await one('UPDATE luce_horoscopes SET published=NULL,published_at=NULL,scheduled_at=NULL,scheduled_draft=NULL,scheduled_title=NULL,scheduled_demo=NULL,revision=revision+1 WHERE month=$1 AND revision=$2 RETURNING *',[req.params.month,req.body.revision]);if(!result)throw fail('This month changed. Reload first.',409);res.json(result);});
 }
 return {initialize,register,access,syncSubscription,signs,tick,start,stop};
}

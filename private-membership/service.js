...
function createMembership({pool,getMailer,env=process.env,clock=()=>new Date(),stripeClient,fetcher=fetch}){
 ...
 function register(app,checkAdmin){
  ...
  app.get('/api/membership/config',wrap(async(req,res)=>{const m=await session(req),p=await preview(req);await tick();const next=await one('SELECT month,scheduled_at FROM luce_horoscopes WHERE scheduled_at IS NOT NULL AND scheduled_demo=false ORDER BY scheduled_at LIMIT 1');const latest=await one('SELECT month FROM luce_horoscopes WHERE published IS NOT NULL AND published_demo=false ORDER BY month DESC LIMIT 1');res.json({private:!liveEnabled(),purchasingEnabled:!!liveEnabled(),testCheckoutEnabled:!!p&&testCheckoutEnabled(),foundingOfferOpen:env.MEMBERSHIP_FOUNDING_OFFER_OPEN==='true',testEmail:p?.email||'',turnstileSiteKey:liveEnabled()?env.TURNSTILE_SITE_KEY:null,signs,nextPublication:next||null,latestMonth:latest?.month||null});}));
  ...
  app.get('/api/membership/horoscopes',needPreview,wrap(async(req,res)=>{await tick();const m=await reconcile(await needMember(req));if(!access(m))throw fail('Confirm your account and check your membership access on the Account page.',403);res.json((await q('SELECT month,published_title AS title,published AS content,published_demo AS demo,published_at FROM luce_horoscopes WHERE published IS NOT NULL AND ($1 OR published_demo=false) ORDER BY month DESC',[m.is_test])).rows);}));
  ...
  app.get('/api/membership/reading',needPreview,wrap(async(req,res)=>{await tick();const m=await reconcile(await needMember(req));if(!access(m))throw fail('Your membership does not currently include content access.',403);if(!/^\d{4}-\d{2}$/.test(String(req.query.month||'')))throw fail('Choose a published month.');const month=await one('SELECT month,published_title AS title,published AS content,published_demo AS demo FROM luce_horoscopes WHERE month=$1 AND published IS NOT NULL',[req.query.month]);if(!month||month.demo&&!m.is_test)throw fail('No published horoscope is available for this month.',404);res.json({month:month.month,title:month.title,demo:month.demo,groups:chart.matchingReading(m.birth_chart,month)});}));

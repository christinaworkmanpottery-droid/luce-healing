...
test('separate list/editor screens, blank new guidance, correct record month and Pacific scheduling',async()=>{
 ...
 assert.deepEqual([...doc.querySelectorAll('#mb-primary button')].map(b=>b.textContent),['Save Draft','Publish']);
 ...
 d.window.close();d=await editor(h);d.window.document.querySelector(`[data-open-month="${row.month}"]`).click();assert.equal(d.window.document.querySelector('#mb-publish-time').value,'2026-10-01T09:00');assert.equal(d.window.document.querySelector('#mb-scheduling').open,true);

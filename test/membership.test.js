...
test('Pacific schedules publish atomically without an admin session; edits, cancellation, drafts and emails remain controlled',async()=>{const h=await harness();try{
 ...
 const live=(await h.db.query("SELECT * FROM luce_horoscopes WHERE month='2026-10'")).rows[0];assert.equal(signs.filter(s=>typeof live.published[s]==='string').length,12);assert.equal(live.published.General,'');assert.equal(live.published.Aries,'Revised Aries');assert.equal(live.scheduled_at,null);

(() => {
 ...
 const stateLabel=value=>({legacy_unverified:'Legacy — needs confirmation',verified:'Active / verified',pending:'Pending confirmation',unsubscribed:'Unsubscribed',bounced:'Bounced',invalid:'Invalid',spam:'Spam / Archived',accepted:'Accepted by provider',unknown:'Uncertain — needs review'}[value]||value);
 ...
 function subscribersView(){
  ...
  const rows=subscribers.filter(s=>(filter?s.status===filter:s.status!=='spam')&&(!search||(s.email+' '+(s.name||'')).toLowerCase().includes(search)));

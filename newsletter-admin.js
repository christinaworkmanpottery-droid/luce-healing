(() => {
 ...
 const stateLabel=value=>({legacy_unverified:'Legacy — needs confirmation',verified:'Active / verified',pending:'Pending confirmation',unsubscribed:'Unsubscribed',bounced:'Bounced',invalid:'Invalid',spam:'Spam / Archived',archived:'Archived — administrative cleanup',accepted:'Accepted by provider',unknown:'Uncertain — needs review'}[value]||value);
 ...
 function subscribersView(){
  ...
  const rows=subscribers.filter(s=>(filter?s.status===filter:!['spam','archived'].includes(s.status))&&(!search||(s.email+' '+(s.name||'')).toLowerCase().includes(search)));
 ...
 async function load(){
  if(!password())return;
  const [config,rows,posts]=await Promise.all([api('newsletter/status'),api('newsletter/subscribers?include_archived=true'),api('blog')]);
 ...
 root.addEventListener('click',event=>{
  ...
  perform(async()=>{
   if(b.dataset.archive||b.dataset.restore){const archiving=!!b.dataset.archive;if(!confirm(archiving?'Archive this address as spam? It will be hidden and blocked from newsletter, blog and verification emails. You can restore it later.':'Restore this record? It will remain excluded from sends until email confirmation; unsubscribed addresses stay unsubscribed.'))return;await api('newsletter/subscribers/'+(b.dataset.archive||b.dataset.restore)+'/archive','POST',{archived:archiving});subscribers=await api('newsletter/subscribers?include_archived=true');subscribersView();message(archiving?'Archived as spam; record preserved and emailing blocked.':'Restored; no email sent. Confirmation is required before subscribing.');}
   if(b.dataset.unsubscribe){if(!confirm('Unsubscribe this address from marketing emails? The record will be preserved.'))return;await api('newsletter/'+b.dataset.unsubscribe,'DELETE');subscribers=await api('newsletter/subscribers?include_archived=true');subscribersView();message('Unsubscribed; record preserved.');}
